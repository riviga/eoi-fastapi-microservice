...
def get_by_id(id: int):
    return get_pedido_by_id_query(id).first()        


def get_pedido_by_id_query(id: int):    
    pedido_id_query = db.query(PedidoDB).filter(PedidoDB.id == id)
    if pedido_id_query.first() is None:
        raise HTTPException(status_code=404, detail=f"Fármaco {id} no encontrado")
    return pedido_id_query
...
def delete(id: int):
    farmaco_id_query = get_pedido_by_id_query(id)            
